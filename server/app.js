...
app.get("/games", async (req, res) => {
    try { //Försöker läsa in från filen games.json
...
app.post("/games", async (req, res) => {
    try{
        ...
        if (currentGames && currentGames.length >0){
            maxGameID= currentGames.reduce((maxId, currentElement)=> currentElement.id> maxId? 
            currentElement.id: maxId, maxTaskId); //ifsats som håller koll på att man inte duplicerar idn
            
        } 
        const newGame= {id :maxTaskId+ 1,...games};
...
app.delete('/games/:id', async (req, res) => {
    try{
        ...
        if (currentGames.length > 0){ //checkar att den är längre än 0, dvs inte tom.
            //Skriver över filen med allt förutom det id vi vill ta bort.
            await fs.writeFile('./games.json', JSON.stringify(currentGames.filter((game) => game.id != id)));
            res.send({ message: `Spelet med id ${id} togs bort`});
        } else {
            res.status(404).send({error: 'Inget spel att t bort'});
        }
    } catch(error){
